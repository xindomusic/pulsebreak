extends SceneTree
## Records the actual native Master mix, after the production limiter.
...
func capture_audio() -> void:
	output_dir = ProjectSettings.globalize_path("res://qa")
	...
	if not failure.is_empty():
		...
		return
	for stem: String in ["foundation", "drive", "lead"]:
		if not ResourceLoader.exists("res://assets/audio/music_resonance_%s.ogg" % stem):
			failure = "Missing imported Resonance music stem: " + stem
	if not ResourceLoader.exists("res://assets/audio/pulse_resonance.wav"):
		failure = "Missing imported Resonance pulse sound."
...
func finish_capture() -> void:
	...
	var wav_path := output_dir.path_join("resonance-engine-audio.wav")
	if recording:
